...
int main(int argc, char **argv) {
    ...
    if(!fs) {
        fprintf(stderr, "Oops! Failed to connect to maprfs!\n");
        exit(-1);
    } 
    ...
    if (!maprfs_only) {
      ...
      if(!lfs) {
          fprintf(stderr, "Oops! Failed to connect to 'localfs' !\n");
          exit(-1);
      } 
    }
        ...
        const char* writePath = "/tmp/testfile.txt";
    {
        ...
        if ((currentPos = hdfsTell(fs, writeFile)) == (tOffset)-1) {
            fprintf(stderr, 
    ...
    int result = 0;
    {
        ...
        /* MapR: NOTE: In maprfs, replication cannot be set on a file, rather on a volume */
#if 0
    ...
    // MapR NOTE: We don't allow switching to a different user.
    // One can always use sudo to test as a different user
#if 0
...

    {
      ...
      if(!fs) {
        fprintf(stderr, "Oops! Failed to connect to maprfs as user %s!\n",tuser);
        exit(-1);
      } 
        ...
        hdfsFileInfo *finfo = hdfsGetPathInfo(fs, writePath);
        fprintf(stderr, "maprfs new file user is correct: %s\n", ((result = (strcmp(finfo->mOwner, tuser) != 0)) ? "Failed!" : "Success!"));
        totalResult += result;
    }
#endif

    if (maprfs_only) {
      tSize size = hdfsGetNameContainerSizeBytes(fs, "/");
      fprintf(stderr, "Size of Name Container for volume mapr.cluster.root = %d bytes\n", size);
      totalResult += (size < 0);
    }
